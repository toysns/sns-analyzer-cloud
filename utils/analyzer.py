...
このアカウントを「もっと伸ばすにはどうすべきか」を分析してください。
マネタイズの話は求められない限りしないでください。フォロワー増加・再生数向上・エンゲージメント改善に集中してください。

以下の優先順位で分析・提案を行ってください:

### 1. コンセプト診断（最重要）
- このアカウントの需要A（ターゲットが本当に求めているもの）は何か？
- 変数/定数の設計は適切か？何を変えるべきで何を固定すべきか？
- 「誰に・何を・どうやって」が明確か？改善案があれば具体的に提示

### 2. コンテンツ改善（台本・構成）
- 伸びている投稿と伸びていない投稿の**具体的な違い**を言語化
- 冒頭フック（最初の1-2秒）の改善案を3つ以上
- 台本構成の改善：上の句/下の句メソッド、ネガティビティ構文、ギャップ構文など具体的な型を提案
- テロップの使い方、情報密度の改善案

### 3. フォーマット提案
- フォーマット辞書を参照し、このアカウントに最適なフォーマットを具体的に提案
- 今使っているフォーマットの問題点と代替案
- 実際の台本例（冒頭5行程度）を書いて見せる

### 4. 投稿戦略
- 投稿頻度・曜日・時間帯の最適化提案（トレンドデータがあれば根拠付き）
- 次の10投稿で試すべきテーマ案を具体的に3つ以上

### 5. 競合との差別化
- 同ジャンルでよく見るパターンと、このアカウントが差別化できるポイント
- 「この方向に振ればブルーオーシャンになる」というポジショニング提案

**出力ルール:**
- 抽象的なアドバイスは禁止。必ず「具体的に何をどう変えるか」を書く
- 「〇〇した方がいいです」ではなく「〇〇を△△に変えてください。理由は〜」
- 改善案には必ず理由（データ・事例・心理学的根拠）をつける
- 伸びている投稿の具体的な要素を引用して説明する""",
}
